...
async function getClicksPerLink(username, time) {
  ...
  const response = await apiClient.get(`/clicks/per-link/${username}`, {
    params,
  });
  return response.data;
}
...
async function getClickTimeline(username, time) {
  const params = time ? { time } : {};
  const response = await apiClient.get(`/clicks/timeline/${username}`, {
    params,
  });
  return response.data;
}
...
async function getProfileVisitTimeline(username, time) {
  const params = time ? { time } : {};
  const response = await apiClient.get(`/profile-visits/timeline/${username}`, {
    params,
  });
  return response.data;
}
